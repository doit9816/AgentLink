...
- Add a release-time staging step that copies `latest.json`, installer artifacts, and signature files into a public updater-feed directory and rewrites metadata URLs to that public base URL.
- Extend the release workflow to publish the staged updater feed to the `updater-feed` branch on tagged releases.
- Update release and updater documentation so maintainers know the new public feed path and deployment expectations.

## Capabilities

### New Capabilities
- `desktop-updater-feed`: Desktop auto-update uses a public static feed whose metadata and installer URLs stay reachable without GitHub Release asset access.

### Modified Capabilities
- `build-test-workflow`: Release publishing now includes deployment of updater metadata and referenced artifacts to a public static host in addition to producing signed bundles.
